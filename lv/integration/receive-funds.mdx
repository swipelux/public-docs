...
### Pieprasiet norēķināšanās preferenci kartei, Apple Pay vai Google Pay

Kartes kotācijai tajā pašā pieprasījumā varat iekļaut neobligātu `paymentMethod` ar vērtību `card`, `apple_pay` vai `google_pay`. Mitinātā norēķināšanās atveras ar šo metodi, ja tā ir pieejama klientam un viņa ierīcei, un klients joprojām var izvēlēties jebkuru citu metodi, ko piedāvā norēķināšanās. Izlaidiet lauku, lai saglabātu noklusējuma preferenci.
...
`paymentMethod` ir preference, nevis garantija. Apple Pay un Google Pay pieejamība ir atkarīga no klienta, viņa ierīces un norēķināšanās, tāpēc metodes pieprasīšana nepadara to pieejamu. Preference nemaina kotācijas iespēju, summas vai maksas, un norēķināšanās apstiprina galīgās maksas. Pārveduma nolasījumi atgriež pieprasīto vērtību; tie neapstiprina, kuru metodi klients izmantoja. Ja `paymentMethod` tiek nosūtīts kotācijai, kas nav kartes kotācija, pieprasījums neiztur validāciju, nevis tiek ignorēts, un atkārtojumam ar to pašu idempotences atslēgu jānosūta tas pats `paymentMethod`.
