...
Katrs ieraksts sarakstos `verificationSessions` un `tosSessions` publicē skaidru `action` objektu. Nolasiet `action.kind` pirms klienta nosūtīšanas; nesecīniet saites pieejamību no sesijas `status`.

- `action.kind: "available"` ietver `action.url` un `action.expiresAt` (pašlaik vienmēr `null`). Nosūtiet klientu uz `action.url`, pēc tam atkārtoti nolasiet uzdevumu. Sesijas plakanie lauki `url` un `expiresAt` ir pieejamās darbības saderības spoguļi.
- `action.kind: "unavailable"` nozīmē, ka nevajadzētu piedāvāt nevienu saiti. Atkārtoti izmantotas, pabeigtas, noraidītas un atceltas sesijas publicē šo formu, un plakanie lauki `url` un `expiresAt` tiek izlaisti.
