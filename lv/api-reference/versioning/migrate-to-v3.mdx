...
description: "Divu piegājienu ceļvedis migrācijai no Swipelux API v1 un v2 uz v3: vispirms objektu modeļa koncepcijas, pēc tam galapunktu kartējumi un kontrolsaraksts."
...
| `POST /v2/.../documents/upload-token` plus `POST /v2/documents/direct-upload` | Vairs nav, augšupielādē tieši ar savu API atslēgu (zemāk) |
...
- **Neapstrādāta failu glabāšana**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (plus `/{documentId}`), augšupielādē vienreiz ar savu API atslēgu, tad atsaucies uz dokumenta id iesniegumu atbildēs. Tas aizstāj katru upload-token un direct-upload uzņemšanu.
