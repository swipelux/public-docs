---
title: "Migrace na v3"
description: "Migrujte stávající integraci v1 nebo v2 na Swipelux API v3 ve dvou fázích: nejprve koncept, poté endpoint po endpointu."
...
| `POST /v2/.../documents/upload-token` plus `POST /v2/documents/direct-upload` | Nahrávejte přímo se svým API klíčem (níže), nebo do pěti minut pošlete čerstvý direct-upload `storageKey` jako JSON na `POST /v3/.../documents` |
...
- **Úložiště surových souborů**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (plus `/{documentId}` a `GET .../{documentId}/download`), nahrajte jednou se svým API klíčem, poté odkazujte na document ids v odpovědích submission. Toto nahrazuje každý upload-token a direct-upload intake. `POST .../documents` přijímá také JSON tělo (`storageKey`, `fileName`, `type`), které spotřebuje stále čerstvou referenci v rozsahu zákazníka z `POST /v2/documents/direct-upload`, takže zaparkovaná nahrání se migrují bez opětovného posílání bajtů.
