...
description: "Vytvořte jednorázový fiat pay-in Swipelux, zobrazte přesné bankovní nebo kartové pokyny a vypořádejte přijaté prostředky do peněženky zákazníka."
---
...
Bankovní pokyny mohou u metod `ach`, `wire`, `rtp` a `swift` obsahovat také volitelná pole `bankName`, `bankAddress`, `accountHolderName` a `accountHolderAddress`. Každé vrácené pole zobrazte vedle ostatních bankovních souřadnic. Odesílající banky často vyžadují adresu přijímající banky pro `swift` a jiné mezinárodní převody, proto `bankAddress` zobrazte vždy, když je vráceno.
