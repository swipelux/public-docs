...
description: "Vytvořte jednorázový fiat pay-in ve Swipelux, zobrazte přesné bankovní či karetní pokyny a vypořádejte přijaté prostředky do peněženky zákazníka."
---
...
### Přidejte návratovou URL pro kartu a Apple Pay

Pro quote kartou nebo Apple Pay můžete do stejného požadavku zahrnout volitelné `redirectUrl`. Poté, co zákazník dokončí hostovaný checkout, stránka checkoutu otevře tuto URL a vrátí zákazníka zpět do vaší aplikace.
...
`redirectUrl` musí být absolutní HTTPS URL o délce nejvýše 2048 znaků a nesmí obsahovat vložené přihlašovací údaje. Odeslání `redirectUrl` pro quote, který nepodporuje návratovou navigaci, selže při validaci, místo aby bylo ignorováno.

Příchod na `redirectUrl` je pouze navigace prohlížeče. Nepotvrzuje, že platba nebo převod byly dokončeny. Dokončení určujte ze stavu převodu v kroku 4 a z webhookových událostí.

### Předvyberte metodu checkoutu pro quote kartou

Pro quote kartou můžete do stejného požadavku zahrnout volitelné `checkoutMethod`, kterým zvolíte, s jakou možností financování se hostovaný checkout otevře. Aktuální hodnoty jsou `card`, `apple_pay`, `google_pay`, `cash_app`, `sepa` a `blik`. Seznam je otevřený a může se rozšiřovat, proto při čtení převodu tolerujte hodnoty, které neznáte. Checkout se otevře s vyžádanou metodou, pokud je pro zákazníka a jeho zařízení dostupná, a zákazník si stále může vybrat kteroukoli jinou metodu, kterou checkout nabízí. Pokud pole vynecháte, zachová se výchozí předvýběr.
...
`checkoutMethod` je preference, nikoli záruka. Dostupnost závisí na zákazníkovi, jeho zařízení a checkoutu, takže vyžádání metody ji nezpřístupní. `sepa` a `blik` jsou zde možnosti financování uvnitř kartového checkoutu, nikoli schopnosti, raily ani účty Swipelux. Předvýběr nemění schopnost, částky ani poplatky quote a konečné poplatky potvrzuje checkout. Čtení převodu vrací vyžádanou hodnotu; nepotvrzuje, kterou metodu zákazník použil. Odeslání `checkoutMethod` pro quote, který není kartový, selže při validaci, místo aby bylo ignorováno, a opakování se stejným idempotentním klíčem musí odeslat stejné `checkoutMethod`.
