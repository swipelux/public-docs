...
### Vyžádejte si preferenci checkoutu pro kartu, Apple Pay nebo Google Pay

Pro quote kartou můžete do stejného požadavku zahrnout volitelné `paymentMethod` s hodnotou `card`, `apple_pay` nebo `google_pay`. Hostovaný checkout se otevře s touto metodou, pokud je pro zákazníka a jeho zařízení dostupná, a zákazník si stále může vybrat kteroukoli jinou metodu, kterou checkout nabízí. Pokud pole vynecháte, zachová se výchozí preference.
...
`paymentMethod` je preference, nikoli záruka. Dostupnost Apple Pay a Google Pay závisí na zákazníkovi, jeho zařízení a checkoutu, takže vyžádání metody ji nezpřístupní. Preference nemění schopnost, částky ani poplatky quote a konečné poplatky potvrzuje checkout. Čtení převodu vrací vyžádanou hodnotu; nepotvrzuje, kterou metodu zákazník použil. Odeslání `paymentMethod` pro quote, který není kartový, selže při validaci, místo aby bylo ignorováno, a opakování se stejným idempotentním klíčem musí odeslat stejné `paymentMethod`.
