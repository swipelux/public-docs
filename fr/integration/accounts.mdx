...
Chaque endpoint de compte est rattaché à `/v3/customers/{customerId}/accounts/{accountId}`. Il n'existe pas de recherche inverse d'un `accountId` vers un `customerId`. Stockez le `customerId` aux côtés de chaque `accountId` que vous recevez afin que les lectures, devis et transferts ultérieurs puissent adresser le compte.
