...
### Présélectionner la méthode de paiement hébergé pour un devis par carte

Pour un devis par carte, vous pouvez inclure un `checkoutMethod` facultatif dans la même requête afin de choisir l'option de financement sur laquelle s'ouvre le paiement hébergé. Les valeurs actuelles sont `card`, `apple_pay`, `google_pay`, `cash_app`, `sepa` et `blik`. La liste est ouverte et peut s'étendre ; tolérez donc les valeurs que vous ne reconnaissez pas dans les lectures du transfert. Le paiement hébergé s'ouvre sur la méthode demandée lorsqu'elle est disponible pour le client et son appareil, et le client peut toujours choisir toute autre méthode proposée. Omettez-le pour conserver la présélection par défaut.
...
`checkoutMethod` est une préférence, pas une garantie. La disponibilité dépend du client, de son appareil et du paiement hébergé ; demander une méthode ne la rend pas disponible. Ici, `sepa` et `blik` sont des options de financement à l'intérieur du paiement par carte hébergé, et non des capacités, des rails ou des comptes Swipelux. La présélection ne modifie ni la capacité, ni les montants, ni les frais du devis, et le paiement hébergé confirme les frais définitifs. Les lectures du transfert renvoient la valeur demandée ; elles ne confirment pas la méthode réellement utilisée par le client. Envoyer `checkoutMethod` pour un devis qui n'est pas un devis par carte échoue à la validation au lieu d'être ignoré, et une relecture avec la même clé d'idempotence doit envoyer le même `checkoutMethod`.
