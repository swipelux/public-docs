...
| `POST /v2/.../documents/upload-token` plus `POST /v2/documents/direct-upload` | Chargez directement avec votre clé API (ci-dessous), ou envoyez un `storageKey` de direct-upload récent en JSON à `POST /v3/.../documents` dans les cinq minutes |
...
- **Stockage de fichiers bruts** : `POST/GET/DELETE /v3/customers/{customerId}/documents` (plus `/{documentId}` et `GET .../{documentId}/download`), chargez une fois avec votre clé API puis référencez les ids de document dans les réponses de submission. Cela remplace tous les intakes upload-token et direct-upload. `POST .../documents` accepte aussi un corps JSON (`storageKey`, `fileName`, `type`) qui consomme une référence à portée client encore récente issue de `POST /v2/documents/direct-upload`, de sorte que les téléversements mis en attente migrent sans renvoyer les octets.
