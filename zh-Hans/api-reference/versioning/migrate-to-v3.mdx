...
description: "两步走的迁移指南，将 Swipelux API v1 和 v2 迁移到 v3：先掌握对象模型概念，再进行端点映射、状态机与检查清单。"
sidebarTitle: "迁移到 v3"
---
...
v1 和 v2 中演化出了四种彼此重叠的方式来让客户具备支付能力：`/rails`、`/banks`、`/accounts/applications`，以及企业客户的 `rail-applications` 接口，每一种都有各自的一套状态词汇。文档采集（`/documents`、KYC 导入、验证 SDK token）也与它真正解锁的对象脱节。v3 把所有这些整合为六个资源，即客户加上其所拥有的五种事物：
...
- 一个 task 携带 `requirements[]`，即具体的问询项。每一项都有 task 内唯一的 `requirementId`、一个用于命名问询的稳定 `key`（例如地址证明，UI 中据此去重），以及一个类型化的 `request`，精确描述所需输入的类型（text、date、select、document、attestation 等）。
