...
Jeder Eintrag in `verificationSessions` und `tosSessions` veröffentlicht ein explizites `action`-Objekt. Lesen Sie `action.kind`, bevor Sie den Kunden weiterleiten; leiten Sie die Verfügbarkeit des Links nicht aus dem `status` der Session ab.

- `action.kind: "available"` enthält `action.url` und `action.expiresAt` (derzeit immer `null`). Senden Sie den Kunden an `action.url` und lesen Sie die Aufgabe dann erneut. Die flachen Felder `url` und `expiresAt` auf der Session sind Kompatibilitäts-Spiegel der verfügbaren Aktion.
- `action.kind: "unavailable"` bedeutet, dass kein Link angezeigt werden soll. Wiederverwendete, abgeschlossene, abgelehnte und stornierte Sessions veröffentlichen diese Form, und die flachen Felder `url` und `expiresAt` entfallen.
