...
description: "Wählen Sie zwischen kundeneigenen Wallets, Bankkonten und Drittparteien-Zielen, die jeder Swipelux-Pay-in-, Payout- oder Issuing-Flow benötigt."
---
...
Listen-, Erstellen-, Lesen-, Aktualisieren- und Archivieren-Operationen für Kundenkonten liegen unter `/v3/customers/{customerId}/accounts`; Operationen für ein einzelnes Konto hängen `/{accountId}` an. Es gibt keine Rückwärtssuche von `accountId` zu `customerId`. Speichern Sie die `customerId` zusammen mit jeder `accountId`, damit spätere Anfragen das Konto adressieren können.
...
Bevor du den **First pay-in**-Assistenten im Händler-Panel für Karten oder Apple Pay startest, erstelle eine Polygon-Wallet für diesen Kunden. Der Assistent bietet nur diese Polygon-Wallet an und kann ohne sie nicht fortfahren. Diese Voraussetzung gilt für den Assistenten im Händler-Panel; direkte API-Integrationen dürfen nur Währungen, Netzwerke, Methoden und Kontotypen verwenden, die für den aktuellen Kunden und die aktuelle Funktion zurückgegeben werden.
