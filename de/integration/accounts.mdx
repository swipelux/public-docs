...
Jeder Konto-Endpoint liegt unter `/v3/customers/{customerId}/accounts/{accountId}`. Eine Rückwärtssuche von `accountId` zu `customerId` gibt es nicht. Speichern Sie die `customerId` zusammen mit jeder `accountId`, die Sie erhalten, damit spätere Reads, Quotes und Transfers das Konto ansprechen können.
