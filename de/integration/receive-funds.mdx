...
### Die Checkout-Methode für ein Karten-Quote vorauswählen

Für ein Karten-Quote können Sie in derselben Anfrage optional ein `checkoutMethod` angeben, um festzulegen, mit welcher Zahlungsoption sich der gehostete Checkout öffnet. Die aktuellen Werte sind `card`, `apple_pay`, `google_pay`, `cash_app`, `sepa` und `blik`. Die Liste ist offen und kann wachsen; tolerieren Sie daher bei Transfer-Lesevorgängen Werte, die Sie nicht kennen. Der Checkout öffnet sich mit der angeforderten Methode, wenn sie für den Kunden und sein Gerät verfügbar ist, und der Kunde kann weiterhin jede andere Methode wählen, die der Checkout anbietet. Lassen Sie das Feld weg, um die Standard-Vorauswahl beizubehalten.
...
`checkoutMethod` ist eine Präferenz, keine Garantie. Die Verfügbarkeit hängt vom Kunden, seinem Gerät und dem Checkout ab; eine Anforderung macht die Methode nicht verfügbar. `sepa` und `blik` sind hier Zahlungsoptionen innerhalb des Karten-Checkouts, keine Fähigkeiten, Rails oder Konten von Swipelux. Die Vorauswahl ändert weder die Fähigkeit noch die Beträge oder Gebühren des Quotes, und der Checkout bestätigt die endgültigen Gebühren. Transfer-Lesevorgänge geben den angeforderten Wert zurück; sie bestätigen nicht, welche Methode der Kunde verwendet hat. Das Senden von `checkoutMethod` für ein Quote, das kein Karten-Quote ist, schlägt bei der Validierung fehl, statt ignoriert zu werden, und eine Wiederholung mit demselben Idempotenzschlüssel muss dasselbe `checkoutMethod` senden.
