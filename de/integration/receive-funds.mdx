...
### Eine Checkout-Präferenz für Karte, Apple Pay oder Google Pay anfordern

Für ein Karten-Quote können Sie in derselben Anfrage optional ein `paymentMethod` mit dem Wert `card`, `apple_pay` oder `google_pay` angeben. Der gehostete Checkout öffnet sich mit dieser Methode, wenn sie für den Kunden und sein Gerät verfügbar ist, und der Kunde kann weiterhin jede andere Methode wählen, die der Checkout anbietet. Lassen Sie das Feld weg, um die Standardpräferenz beizubehalten.
...
`paymentMethod` ist eine Präferenz, keine Garantie. Die Verfügbarkeit von Apple Pay und Google Pay hängt vom Kunden, seinem Gerät und dem Checkout ab; eine Anforderung macht die Methode nicht verfügbar. Die Präferenz ändert weder die Fähigkeit noch die Beträge oder Gebühren des Quotes, und der Checkout bestätigt die endgültigen Gebühren. Transfer-Lesevorgänge geben den angeforderten Wert zurück; sie bestätigen nicht, welche Methode der Kunde verwendet hat. Das Senden von `paymentMethod` für ein Quote, das kein Karten-Quote ist, schlägt bei der Validierung fehl, statt ignoriert zu werden, und eine Wiederholung mit demselben Idempotenzschlüssel muss dasselbe `paymentMethod` senden.
