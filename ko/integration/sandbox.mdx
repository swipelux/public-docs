...
[`POST /v3/sandbox/tasks`](/api-reference/sandbox/post-v3-sandbox-tasks)로 샌드박스 태스크를 생성하세요. 테스트 중인 리소스에 맞는 `scope.type`을 선택하고, 고객이 제출해야 하는 항목을 설명하는 하나 이상의 `items`를 추가하세요.

`scope.type`은 `customer`, `capability`, `transfer`를 허용해요. `transfer` 스코프에는 `customerId`와 `transferId`가 모두 필요해요. `capability` 스코프에는 `customerId`와 `capabilityId`가 모두 필요해요. `customer` 스코프에는 `customerId`만 필요해요.

`items[].type`은 `document`, `profile`, `questionnaire`, `explanation`, `attestation`을 허용해요. 고객이 파일을 업로드해야 하면 `document`, 특정 고객 프로필 필드에는 `profile`, 자유 텍스트 답변에는 `questionnaire`, 서면 설명에는 `explanation`, 고객이 텍스트를 확인해야 하면 `attestation`을 사용하세요.

태스크를 워크플로우와 일치시키려면 `category`를 설정하세요. 거래 정보 요청(RFI)의 경우 `transfer` 스코프와 함께 `transaction_rfi`를 사용하세요.
...
Document `constraints`는 `acceptedDocumentTypes`, `acceptedFormats`, `maxAgeDays`, `captureMethods`, `nameMatch`를 허용해요. 전체 필드 카탈로그는 [샌드박스 태스크 생성](/api-reference/sandbox/post-v3-sandbox-tasks)을 참고하세요.
