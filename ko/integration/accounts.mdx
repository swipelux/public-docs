---
title: "계좌와 지갑"
description: "각 Swipelux 페이인, 페이아웃, 발급 은행 계좌 흐름에 필요한 고객 소유 지갑, 은행 계좌, 제3자 수취인 목적지 리소스를 비교하고 상황에 맞는 계좌 유형을 선택하는 방법을 API 관점에서 설명하는 계좌 개념 통합 가이드 페이지입니다."
---

모든 계좌는 하나의 고객에게 속해요. 두 가지 차원을 사용해 선택하세요: 누가 관리하는지, 그리고 지갑인지 은행 계좌인지.

고객 계좌의 목록, 생성, 조회, 업데이트, 보관 작업은 `/v3/customers/{customerId}/accounts` 아래에 있고, 개별 계좌 작업에는 `/{accountId}`가 추가돼요. `accountId`에서 `customerId`를 역조회할 수 없어요. 이후 요청에서 계좌를 지정할 수 있도록 각 `accountId`와 `customerId`를 함께 저장하세요.
...
카드 또는 Apple Pay용 판매자 패널의 **First pay-in** 안내를 시작하기 전에 해당 고객에게 Polygon 지갑을 생성하세요. 안내에서는 이 Polygon 지갑만 제공하며 지갑이 없으면 계속할 수 없습니다. 이 전제 조건은 판매자 패널 안내에 적용됩니다. 직접 API 통합에서는 현재 고객 및 기능에 대해 반환된 통화, 네트워크, 메서드 및 계좌 유형만 사용해야 합니다.
