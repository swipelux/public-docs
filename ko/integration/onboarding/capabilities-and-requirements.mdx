...
`verificationSessions`와 `tosSessions`의 각 항목은 명시적인 `action` 객체를 게시해요. 고객을 안내하기 전에 `action.kind`를 확인하세요. 세션 `status`로부터 링크 사용 가능 여부를 추론하지 마세요.

- `action.kind: "available"`은 `action.url`과 `action.expiresAt`을 포함해요(현재는 항상 `null`이에요). `action.url`로 고객을 안내한 뒤 태스크를 다시 조회하세요. 세션의 평탄한 `url`과 `expiresAt` 필드는 available 액션의 호환용 미러예요.
- `action.kind: "unavailable"`은 어떤 링크도 제시하지 말아야 함을 의미해요. 재사용, 완료, 거절, 취소된 세션은 이 형태를 게시하며, 평탄한 `url`과 `expiresAt` 필드는 생략돼요.
