...
API 레퍼런스는 Swipelux API의 공식 계약이에요. 각 동작마다 요청, 응답, 보안, 상태, 오류 스키마가 표시되어 있어요. 전체 공개 문제 카탈로그와 해결 지침은 [오류 및 재시도](/ko/integration/errors)를 참고하세요.
...
동작에 문서화된 문제 스키마를 사용해 입력을 수정할지, 리소스 상태를 새로고침할지, 작업을 완료할지, 재시도할지 결정하세요. 모든 공개 `code`, 안정적인 문제 타입 URI, 재시도 동작, 다음 조치는 [오류 및 재시도](/ko/integration/errors)를 참고하세요.
