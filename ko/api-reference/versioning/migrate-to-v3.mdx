---
title: "v3로 마이그레이션"
description: "기존 v1 또는 v2 통합을 v3 Swipelux API로 두 단계에 걸쳐 마이그레이션합니다. 먼저 개념, 그 다음 엔드포인트별로 진행합니다."
...
| `POST /v2/.../documents/upload-token` 및 `POST /v2/documents/direct-upload` | API 키로 직접 업로드 (아래 참조)하거나, 아직 유효한 direct-upload `storageKey`를 5분 이내에 JSON으로 `POST /v3/.../documents`에 전송 |
...
- **원시 파일 저장소**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (그리고 `/{documentId}` 및 `GET .../{documentId}/download`). API 키로 한 번 업로드하고 submission 답변에서 document id를 참조하십시오. 이것은 모든 upload-token과 direct-upload 인입을 대체합니다. `POST .../documents`는 `POST /v2/documents/direct-upload`에서 아직 유효한 고객 범위 참조를 소비하는 JSON 본문 (`storageKey`, `fileName`, `type`)도 허용하므로, 보관된 업로드는 바이트를 다시 전송하지 않고 마이그레이션됩니다.
