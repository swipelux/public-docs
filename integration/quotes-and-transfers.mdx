...
A quote is a time-bound priced movement. A transfer is the execution of that quote and continues changing after creation.
...
Create a quote with [`POST /v3/quotes`](/api-reference/money-movement/post-v3-quotes). Send one amount side, both currencies, response-derived resource IDs, and a unique `Idempotency-Key`.
