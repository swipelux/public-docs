...
Create a sandbox task with [`POST /v3/sandbox/tasks`](/api-reference/sandbox/post-v3-sandbox-tasks). Pick a `scope.type` that matches the resource you are testing and add one or more `items` describing what the customer must submit.

`scope.type` accepts `customer`, `capability`, or `transfer`. A `transfer` scope requires both `customerId` and `transferId`. A `capability` scope requires both `customerId` and `capabilityId`. A `customer` scope requires only `customerId`.

`items[].type` accepts `document`, `profile`, `questionnaire`, `explanation`, and `attestation`. Use `document` when the customer must upload a file, `profile` for a specific customer profile field, `questionnaire` for a free-text answer, `explanation` for a written explanation, and `attestation` when the customer must confirm text.

Set `category` to align the task with a workflow. For a transaction Request For Information (RFI), use `transaction_rfi` with a `transfer` scope.
...
Document `constraints` accepts `acceptedDocumentTypes`, `acceptedFormats`, `maxAgeDays`, `captureMethods`, and `nameMatch`. See [Create sandbox task](/api-reference/sandbox/post-v3-sandbox-tasks) for the full field catalog.
