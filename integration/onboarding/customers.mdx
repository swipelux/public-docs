...
A customer does not have a verification or readiness status in API v3. Read the customer with [`GET /v3/customers/{customerId}`](/api-reference/customers/get-v3-customers-by-customer-id) for current profile data, then use capabilities and their `openTaskIds` to decide what the customer can do next.
