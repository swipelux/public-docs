...
description: "Discover, request, and activate a customer capability for a payment flow, complete open tasks, and choose between pooled and named account types."
---
...
Bank-account-based capabilities expose two `accountType` values. The capability ID encodes the same choice as a suffix, for example `ach_pooled` or `wire_named`.

- **`pooled`** uses a shared bank account that Swipelux operates. Each pay-in carries a unique per-transfer reference so funds route to the right customer. Use pooled when the customer only needs to fund one-time transfers and does not need reusable bank details to share with a payer.
- **`named`** provisions a customer-specific set of bank details (for example, a virtual IBAN or a dedicated ACH account). The details are reusable and can be shared with any payer. Named is required when the customer needs an [issued bank account](/integration/issue-bank-account) to receive funds directly.

Only pick `named` when the flow you are building needs reusable, customer-specific bank details. Otherwise pick `pooled`. Not every method offers both variants; consult the `supported` response to see what is available for the customer.
