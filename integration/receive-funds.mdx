...
Bank instructions can also include optional `bankName`, `bankAddress`, `accountHolderName`, and `accountHolderAddress` fields on the `ach`, `wire`, `rtp`, and `swift` methods. Render each returned field alongside the other bank coordinates. Sending banks often require the receiving bank address for `swift` and other international wires, so display `bankAddress` whenever it is returned.
