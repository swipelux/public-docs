---
title: "Send funds"
description: "Send from a customer wallet to a customer-owned account or third-party destination."
---
...
Use a customer-owned external bank account when the customer owns the endpoint. Create it with [`POST /v3/customers/{customerId}/accounts`](/api-reference/accounts/post-v3-customers-by-customer-id-accounts) and a unique `Idempotency-Key`:
...
Use [`POST /v3/quotes`](/api-reference/money-movement/post-v3-quotes) with a unique `Idempotency-Key`:
...
Create the transfer with [`POST /v3/transfers`](/api-reference/money-movement/post-v3-transfers). Use one new `Idempotency-Key` for this intended execution.
