...
Every account endpoint is scoped under `/v3/customers/{customerId}/accounts/{accountId}`. There is no reverse lookup from `accountId` to `customerId`. Store the `customerId` alongside every `accountId` you receive so later reads, quotes, and transfers can address the account.
