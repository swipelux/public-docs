...
description: "Choose the customer-owned account or issued resource required by a money flow."
---
...
Use [`POST /v3/customers/{customerId}/accounts`](/api-reference/accounts/post-v3-customers-by-customer-id-accounts). Send a unique `Idempotency-Key` for each intended account creation.
