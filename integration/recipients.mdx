...
`relationship` describes how the recipient relates to your customer. Send one of `employee`, `contractor`, `vendor`, `subsidiary`, `merchant`, `customer`, `landlord`, `family`, or `other`. The field takes only these values; there is no free-text description alongside `other`.
