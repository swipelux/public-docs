...
description: "Create a third-party Swipelux recipient and the bank account or crypto wallet destination that receives a payout, and reuse them across future transfers."
---
...
`relationship` describes how the recipient relates to your customer. Choose a value from the [`POST /v3/customers/{customerId}/recipients` request schema](/api-reference/recipients/post-v3-customers-by-customer-id-recipients). If you send `other`, do not add a companion free-text field; the request schema rejects unknown fields.
