...
| [`capability_direction_unsupported`](#capability-direction-unsupported) | `422` | No | The selected capability does not support the money-movement direction required by the quote or transfer. |
...
| [`capability_snapshot_corrupted`](#capability-snapshot-corrupted) | `500` | No | Swipelux could not evaluate the customer's capability state. |
...
| [`exact_zero_fee_daily_limit_exceeded`](#exact-zero-fee-daily-limit-exceeded) | `409` | No | The request would exceed the customer's daily limit for exact zero-fee payouts. |
| [`exact_zero_fee_payout_activation_unavailable`](#exact-zero-fee-payout-activation-unavailable) | `409` | No | The exact zero-fee payout activation is not available for this transfer. |
| [`exact_zero_fee_quote_invalid`](#exact-zero-fee-quote-invalid) | `409` | No | The exact zero-fee payout quote is invalid or stale. |
...
| [`initial_application_identity_conflict`](#initial-application-identity-conflict) | `409` | No | The initial application identity changed before it was persisted. |
| [`initial_application_owner_unavailable`](#initial-application-owner-unavailable) | `503` | No | No valid owner route is available for the initial application. |
| [`initial_application_scope_unavailable`](#initial-application-scope-unavailable) | `503` | Contextual | Swipelux could not resolve the scope required for the initial application. |
...
| [`ops_rail_handoff_conflict`](#ops-rail-handoff-conflict) | `409` | No | The task submission is blocked by conflicting bank-rail setup. |
...
| [`profile_obligation_crossheal_failed`](#profile-obligation-crossheal-failed) | `500` | No | Swipelux could not update the customer's profile requirements. |
...
| [`quote_amount_out_of_range`](#quote-amount-out-of-range) | `400` | No | The requested payout amount is outside the supported range. |
...
| [`quote_failed`](#quote-failed) | `400`, `500`, `502` | Contextual | Swipelux could not create a usable quote for the request. |
...
| [`quote_source_network_unsupported`](#quote-source-network-unsupported) | `400` | No | The selected source network is not supported for this payout. |
| [`quote_temporarily_unavailable`](#quote-temporarily-unavailable) | `503` | Yes | Payout pricing is temporarily unavailable. |
| [`quote_volume_limit_exceeded`](#quote-volume-limit-exceeded) | `400` | No | The customer's payout volume limit has been reached. |
...
<Accordion title="Relevant API operations (27)">
...
- [`GET /v3/customers/{customerId}/capabilities/{capabilityId}/tasks-preview`](/api-reference/capabilities/get-v3-customers-by-customer-id-capabilities-by-capability-id-tasks-preview) (`getV3CustomersByCustomerIdCapabilitiesByCapabilityIdTasks-preview`, status `400`)
...
<Accordion title="Relevant API operations (56)">
...
<Accordion title="Relevant API operations (23)">
...
- [`GET /v3/customers/{customerId}/documents/{documentId}/download`](/api-reference/documents/get-v3-customers-by-customer-id-documents-by-document-id-download) (`getV3CustomersByCustomerIdDocumentsByDocumentIdDownload`, status `404`)
...
**When it occurs:** A customer or related-party update removes a value that the task in `taskId` has pinned as required.

**What to do:** Read `taskId` and the profile pointers or requirement keys in `fields`, then restore the required values or complete or cancel the task before sending the update again.
...
- `fields`: Profile pointers or requirement keys blocked by the open task.
...
**HTTP status:** `500`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/profile-obligation-crossheal-failed`

**Meaning:** Swipelux could not update the customer's profile requirements.

**When it occurs:** A customer or related-party update cannot reconcile the profile change with its current task requirements.

**What to do:** Retain `correlationId` and contact Swipelux support. Do not repeatedly retry the unchanged update.
...
**HTTP status:** `500`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/capability-snapshot-corrupted`

**Meaning:** Swipelux could not evaluate the customer's capability state.

**When it occurs:** A capability-related operation encounters an invalid stored capability snapshot.

**What to do:** Retain `correlationId` and contact Swipelux support. Do not repeatedly retry the unchanged request.

**Retry guidance:** Do not retry the unchanged request. Apply the action above before sending another request.

<Accordion title="Relevant API operations (11)">
...
**HTTP status:** `409`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/initial-application-identity-conflict`

**Meaning:** The initial application identity changed before it was persisted.

**When it occurs:** Customer creation or a capability request finds conflicting identity for the initial application it is creating.

**What to do:** Retain `correlationId` and contact Swipelux support before retrying the capability request.
...
**HTTP status:** `503`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/initial-application-owner-unavailable`

**Meaning:** No valid owner route is available for the initial application.

**When it occurs:** A capability request cannot assign the initial application to a supported institution option.

**What to do:** Choose a supported institution returned by the capability catalog, or retain `correlationId` and contact Swipelux support when none is available.
...
**HTTP status:** `503`<br />
**Retryable:** Contextual<br />
**Problem type:** `https://docs.swipelux.com/errors/initial-application-scope-unavailable`

**Meaning:** Swipelux could not resolve the scope required for the initial application.

**When it occurs:** Customer creation or a capability request cannot resolve the API-key environment and compliance context needed to create the initial application.

**What to do:** Verify the API-key environment and retain `correlationId`. Follow `retryable` when it is present; otherwise contact Swipelux support before retrying.
...
**HTTP status:** `409`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/ops-rail-handoff-conflict`

**Meaning:** The task submission is blocked by conflicting bank-rail setup.

**When it occurs:** A document task submission cannot reconcile the customer's current bank-rail state.

**What to do:** Keep the uploaded document identifiers, retain `correlationId`, and contact Swipelux support before retrying the same submission. Do not upload replacement copies solely because of this conflict.
...
**HTTP status:** `422`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/capability-direction-unsupported`

**Meaning:** The selected capability does not support the money-movement direction required by the quote or transfer.

**When it occurs:** Quote creation infers a direction the selected capability does not advertise, or transfer creation finds that the quote's pinned capability no longer supports its stored direction.

**What to do:** Choose a capability that supports the intended direction and request a new quote before creating the transfer.
...
<a id="exact-zero-fee-daily-limit-exceeded" className="block scroll-mt-32">
  <span className="sr-only">exact_zero_fee_daily_limit_exceeded error</span>
</a>

### `exact_zero_fee_daily_limit_exceeded`

**HTTP status:** `409`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/exact-zero-fee-daily-limit-exceeded`

**Meaning:** The request would exceed the customer's daily limit for exact zero-fee payouts.

**When it occurs:** A payout quote or transfer priced under an exact zero-fee activation would push the customer's payout total for the current UTC day above the activation's daily cap.

**What to do:** Create a smaller payout, or wait for the UTC day to reset and request a new quote.
...
<a id="exact-zero-fee-payout-activation-unavailable" className="block scroll-mt-32">
  <span className="sr-only">exact_zero_fee_payout_activation_unavailable error</span>
</a>

### `exact_zero_fee_payout_activation_unavailable`

**HTTP status:** `409`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/exact-zero-fee-payout-activation-unavailable`

**Meaning:** The exact zero-fee payout activation is not available for this transfer.

**When it occurs:** Transfer creation executes a payout quote priced under an exact zero-fee activation that is no longer available for the customer.

**What to do:** Create a new payout quote and execute that quote.
...
<a id="exact-zero-fee-quote-invalid" className="block scroll-mt-32">
  <span className="sr-only">exact_zero_fee_quote_invalid error</span>
</a>

### `exact_zero_fee_quote_invalid`

**HTTP status:** `409`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/exact-zero-fee-quote-invalid`

**Meaning:** The exact zero-fee payout quote is invalid or stale.

**When it occurs:** Transfer creation executes an exact zero-fee payout quote whose pricing can no longer be honored.

**What to do:** Create a new payout quote and execute it before it expires.
...
<a id="quote-amount-out-of-range" className="block scroll-mt-32">
  <span className="sr-only">quote_amount_out_of_range error</span>
</a>
...
**HTTP status:** `400`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/quote-amount-out-of-range`

**Meaning:** The requested payout amount is outside the supported range.

**When it occurs:** Payout quote creation requests an amount below the minimum or above the maximum accepted for the selected payout. The response does not state the exact limits.

**What to do:** Change the amount and request a new quote, or contact support for the applicable limits.
...
<a id="quote-source-network-unsupported" className="block scroll-mt-32">
  <span className="sr-only">quote_source_network_unsupported error</span>
</a>

### `quote_source_network_unsupported`

**HTTP status:** `400`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/quote-source-network-unsupported`

**Meaning:** The selected source network is not supported for this payout.

**When it occurs:** Payout quote creation uses a source wallet on a network that the selected payout does not accept.

**What to do:** Choose a source wallet on a supported network, then request a new quote.
...
<a id="quote-temporarily-unavailable" className="block scroll-mt-32">
  <span className="sr-only">quote_temporarily_unavailable error</span>
</a>
...
**HTTP status:** `503`<br />
**Retryable:** Yes<br />
**Problem type:** `https://docs.swipelux.com/errors/quote-temporarily-unavailable`

**Meaning:** Payout pricing is temporarily unavailable.

**When it occurs:** Payout quote creation cannot obtain current pricing at the moment of the request.

**What to do:** Request a new quote after a short delay. A rejected quote request does not move funds.

**Retry guidance:** Retry after a delay. For an idempotent write, reuse the original idempotency key and unchanged request. Retry quote creation after a short delay; do not retry transfer creation automatically.

**Relevant API operations:**

- [`POST /v3/quotes`](/api-reference/money-movement/post-v3-quotes) (`postV3Quotes`, status `503`)

<a id="quote-volume-limit-exceeded" className="block scroll-mt-32">
  <span className="sr-only">quote_volume_limit_exceeded error</span>
</a>

### `quote_volume_limit_exceeded`

**HTTP status:** `400`<br />
**Retryable:** No<br />
**Problem type:** `https://docs.swipelux.com/errors/quote-volume-limit-exceeded`

**Meaning:** The customer's payout volume limit has been reached.

**When it occurs:** Payout quote creation would exceed the volume limit that applies to the customer's payouts.

**What to do:** Contact support before requesting further payout quotes for this customer.
