...
Hver oppføring i `verificationSessions` og `tosSessions` publiserer et eksplisitt `action`-objekt. Les `action.kind` før du sender kunden videre; ikke utled tilgjengelighet av lenken fra sesjonens `status`.

- `action.kind: "available"` inkluderer `action.url` og `action.expiresAt` (for øyeblikket alltid `null`). Send kunden til `action.url`, og les så oppgaven på nytt. De flate feltene `url` og `expiresAt` på sesjonen er kompatibilitetsspeil av den tilgjengelige handlingen.
- `action.kind: "unavailable"` betyr at ingen lenke skal presenteres. Gjenbrukte, fullførte, avviste og kansellerte sesjoner publiserer denne formen, og de flate feltene `url` og `expiresAt` utelates.
