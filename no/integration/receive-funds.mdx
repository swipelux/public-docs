---
title: "Motta midler"
description: "Opprett en engangs Swipelux fiat-pay-in, presenter de nøyaktige bank- eller kortinstruksjonene og gjør opp mottatte midler i kundens wallet."
---

Bruk en prisfestet pay-in når kunden må finansiere et spesifikt beløp. Overføringen gjør opp stablecoin til en utstedt lommebok.

```mermaid
flowchart TD
  A["Ready pay-in capability"] --> B["Ready destination wallet"]
  B --> C["Create quote"]
  C --> D["Execute transfer"]
  D --> E["Display funding instructions"]
  E --> F["Track settlement"]
```

## Før du starter

Du trenger en klar pay-in-capability og en klar utstedt destinasjonslommebok for samme kunde. Fullfør eventuelt åpent arbeid via [Capabilities og oppgaver](/no/integration/onboarding/capabilities-and-requirements), og hent så begge ressursene på nytt.

## 1. Opprett et tilbud

Opprett prisen med [`POST /v3/quotes`](/api-reference/money-movement/post-v3-quotes). Send disse headerne:

```http
X-API-Key: ${SWIPELUX_API_KEY}
Idempotency-Key: payin-quote-001
Content-Type: application/json
```

```json
{
  "customerId": "${CUSTOMER_ID}",
  "capabilityId": "${CAPABILITY_ID}",
  "externalId": "payin-order-123",
  "in": {
    "amount": "150.00",
    "currency": "USD"
  },
  "destinationId": "${SETTLEMENT_WALLET_ID}",
  "out": {
    "currency": "USDC"
  }
}
```

Lagre `data.id` som `QUOTE_ID`. Presenter de returnerte beløpene og gebyrene fremfor å bygge dem opp på nytt fra kursen.

## 2. Utfør tilbudet

Utfør det gjeldende tilbudet før utløp med [`POST /v3/transfers`](/api-reference/money-movement/post-v3-transfers). Bruk en ny nøkkel for denne tiltenkte overføringen:

```http
X-API-Key: ${SWIPELUX_API_KEY}
Idempotency-Key: payin-transfer-001
Content-Type: application/json
```

```json
{
  "quoteId": "${QUOTE_ID}"
}
```

Lagre overføringens `data.id` som `TRANSFER_ID` og behold gjeldende tilstand.

### Legg til en retur-URL for kort og Apple Pay

For et kort- eller Apple Pay-tilbud kan du inkludere en valgfri `redirectUrl` i samme forespørsel. Etter at kunden har fullført den hostede checkouten, åpner checkout-siden denne URL-en for å ta kunden tilbake til appen din.

```json
{
  "quoteId": "${QUOTE_ID}",
  "redirectUrl": "https://your-app.example/payments/complete"
}
```

`redirectUrl` må være en absolutt HTTPS-URL på maksimalt 2048 tegn og kan ikke inneholde innebygde påloggingsopplysninger. Å sende `redirectUrl` for et tilbud som ikke støtter returnavigasjon, feiler i valideringen i stedet for å bli ignorert.

Ankomst til `redirectUrl` er kun nettlesernavigasjon. Den bekrefter ikke at betalingen eller overføringen ble fullført. Avgjør fullføring ut fra overføringens tilstand i trinn 4 og fra webhook-hendelser.

### Forhåndsvelg checkout-metoden for et korttilbud

For et korttilbud kan du inkludere en valgfri `checkoutMethod` i samme forespørsel for å velge hvilket betalingsalternativ den hostede checkouten åpnes med. De gjeldende verdiene er `card`, `apple_pay`, `google_pay`, `cash_app`, `sepa` og `blik`. Listen er åpen og kan vokse, så godta verdier du ikke kjenner igjen i lesing av overføringer. Checkouten åpnes med den forespurte metoden når den er tilgjengelig for kunden og enheten deres, og kunden kan fortsatt velge en hvilken som helst annen metode checkouten tilbyr. Utelat feltet for å beholde standard forhåndsvalg.

```json
{
  "quoteId": "${QUOTE_ID}",
  "checkoutMethod": "apple_pay"
}
```

`checkoutMethod` er en preferanse, ikke en garanti. Tilgjengeligheten avhenger av kunden, enheten deres og checkouten, så det å be om en metode gjør den ikke tilgjengelig. Her er `sepa` og `blik` betalingsalternativer inne i kort-checkouten, ikke Swipelux-kapabiliteter, -skinner eller -kontoer. Forhåndsvalget endrer ikke tilbudets kapabilitet, beløp eller gebyrer, og checkouten bekrefter de endelige gebyrene. Lesing av overføringen returnerer den forespurte verdien; den bekrefter ikke hvilken metode kunden brukte. Å sende `checkoutMethod` for et tilbud som ikke er et korttilbud, feiler i valideringen i stedet for å bli ignorert, og en gjentakelse med samme idempotensnøkkel må sende samme `checkoutMethod`.

## 3. Hent finansieringsinstruksjoner

Les [`GET /v3/transfers/{transferId}/instructions`](/api-reference/money-movement/get-v3-transfers-by-transfer-id-instructions). Render den returnerte `data.instructions`-varianten uten å anta typen.

Disse opplysningene er overføringsspesifikke. Gjenbruk aldri en overførings bankkoordinater, kode, hosted URL, beløp, referanse eller utløp for en annen pay-in.

For bankinstruksjoner: når `reference.required` er true, vis den nøyaktige `reference.value` ved siden av bankkoordinatene og gjør den kopierbar. Vis det returnerte beløpet og valutaen fra samme instruksjonssett.

En utstedt bankkonto er noe annet: den gir gjenbrukbare opplysninger for senere innskudd. Se [Utsted en bankkonto](/no/integration/issue-bank-account) når det er den tiltenkte opplevelsen.

## 4. Følg opp oppgjøret

Les [`GET /v3/transfers/{transferId}`](/api-reference/money-movement/get-v3-transfers-by-transfer-id) etter utførelse og etter hver hendelse. Driv den kundevendte statusen fra siste `data.state`.

Bruk `data.stateDetail` og `data.openTaskIds` når overføringen trenger en ny handling. Ikke merk den som fullført ut fra en forventet plan.

Deretter, legg til [Webhooks](/no/integration/webhooks) og hold overføringsstatusen synkronisert til den når et utfall som produktet ditt håndterer.
