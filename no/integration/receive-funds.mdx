...
### Be om en checkout-preferanse for kort, Apple Pay eller Google Pay

For et korttilbud kan du inkludere en valgfri `paymentMethod` med verdien `card`, `apple_pay` eller `google_pay` i samme forespørsel. Den hostede checkouten åpnes med den metoden når den er tilgjengelig for kunden og enheten deres, og kunden kan fortsatt velge en hvilken som helst annen metode checkouten tilbyr. Utelat feltet for å beholde standardpreferansen.
...
`paymentMethod` er en preferanse, ikke en garanti. Tilgjengeligheten til Apple Pay og Google Pay avhenger av kunden, enheten deres og checkouten, så det å be om en metode gjør den ikke tilgjengelig. Preferansen endrer ikke tilbudets kapabilitet, beløp eller gebyrer, og checkouten bekrefter de endelige gebyrene. Lesing av overføringen returnerer den forespurte verdien; den bekrefter ikke hvilken metode kunden brukte. Å sende `paymentMethod` for et tilbud som ikke er et korttilbud, feiler i valideringen i stedet for å bli ignorert, og en gjentakelse med samme idempotensnøkkel må sende samme `paymentMethod`.
