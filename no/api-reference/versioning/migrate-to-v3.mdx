...
| `POST /v2/.../documents/upload-token` pluss `POST /v2/documents/direct-upload` | Borte, last opp direkte med API-nøkkelen din (under) |
...
- **Rå fillagring**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (pluss `/{documentId}`), last opp én gang med API-nøkkelen din, og referer så til dokument-id-er i submission-svar. Dette erstatter alle upload-token- og direct-upload-inntak.
