...
| `POST /v2/.../documents/upload-token` más `POST /v2/documents/direct-upload` | Sube directamente con tu API key (más abajo), o envía un `storageKey` reciente de direct-upload como JSON a `POST /v3/.../documents` en un plazo de cinco minutos |
...
- **Almacenamiento de archivos en bruto**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (más `/{documentId}` y `GET .../{documentId}/download`), sube una vez con tu API key y luego referencia los ids de documento en las respuestas de submission. Esto reemplaza todos los intakes de upload-token y direct-upload. `POST .../documents` también acepta un cuerpo JSON (`storageKey`, `fileName`, `type`) que consume una referencia con ámbito de cliente aún reciente de `POST /v2/documents/direct-upload`, por lo que las subidas aparcadas migran sin reenviar los bytes.
