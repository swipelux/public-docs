...
description: "Utiliza el contrato de la API, reintenta escrituras de forma segura y gestiona los errores de manera coherente."
---

La Referencia de la API es el contrato autoritativo de la API de Swipelux. Cada operación muestra sus esquemas de solicitud, respuesta, seguridad, estado y errores. Utiliza [Errores y reintentos](/es/integration/errors) para el catálogo público completo de problemas y la guía de resolución.
...
Utiliza los esquemas de problema documentados de la operación para decidir si corregir la entrada, refrescar el estado del recurso, completar una tarea o reintentar. Consulta [Errores y reintentos](/es/integration/errors) para cada `code` público, el URI estable del tipo de problema, el comportamiento de reintento y la siguiente acción.
