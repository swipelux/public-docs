...
Cada entrada en `verificationSessions` y `tosSessions` publica un objeto `action` explícito. Lee `action.kind` antes de enviar al cliente; no deduzcas la disponibilidad del enlace a partir del `status` de la sesión.

- `action.kind: "available"` incluye `action.url` y `action.expiresAt` (actualmente siempre `null`). Envía al cliente a `action.url` y luego vuelve a leer la tarea. Los campos planos `url` y `expiresAt` en la sesión son duplicados de compatibilidad de la acción disponible.
- `action.kind: "unavailable"` significa que no se debe presentar ningún enlace. Las sesiones reutilizadas, completadas, rechazadas y canceladas publican esta forma, y los campos planos `url` y `expiresAt` se omiten.
