...
description: "Crea un pay-in fiat único de Swipelux, presenta las instrucciones exactas de banco o tarjeta y liquida los fondos recibidos en la cartera del cliente."
---
...
Las instrucciones bancarias también pueden incluir los campos opcionales `bankName`, `bankAddress`, `accountHolderName` y `accountHolderAddress` en los métodos `ach`, `wire`, `rtp` y `swift`. Muestra cada campo devuelto junto a las demás coordenadas bancarias. Los bancos emisores suelen exigir la dirección del banco receptor para `swift` y otras transferencias internacionales, así que muestra `bankAddress` siempre que se devuelva.
