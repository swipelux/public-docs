...
### Solicita una preferencia de checkout para tarjeta, Apple Pay o Google Pay

Para una cotización de tarjeta, puedes incluir un `paymentMethod` opcional con el valor `card`, `apple_pay` o `google_pay` en la misma solicitud. El checkout alojado se abre con ese método cuando está disponible para el cliente y su dispositivo, y el cliente aún puede elegir cualquier otro método que ofrezca el checkout. Omítelo para mantener la preferencia predeterminada.
...
`paymentMethod` es una preferencia, no una garantía. La disponibilidad de Apple Pay y Google Pay depende del cliente, de su dispositivo y del checkout, por lo que solicitar uno no lo hace disponible. La preferencia no cambia la capacidad, los importes ni las comisiones de la cotización, y el checkout confirma las comisiones finales. Las lecturas de la transferencia devuelven el valor solicitado; no confirman qué método usó el cliente. Enviar `paymentMethod` para una cotización que no es de tarjeta falla la validación en lugar de ignorarse, y una repetición con la misma clave de idempotencia debe enviar el mismo `paymentMethod`.
