---
title: "API 參考"
description: "使用 API 契約、安全地重試寫入操作,並一致地處理錯誤。"
---

API 參考是 Swipelux API 的權威契約。每個操作都會列出其請求、回應、安全性、狀態與錯誤的結構。完整的公開問題目錄與解決指引,請參閱[錯誤與重試](/zh-Hant/integration/errors)。
...
請依據該操作所記載的問題結構決定要修正輸入、重新載入資源狀態、完成任務或重試。所有公開的 `code`、穩定的問題類型 URI、重試行為與後續行動,請參閱[錯誤與重試](/zh-Hant/integration/errors)。
