---
title: "快速入門"
description: "跟著 Swipelux 快速開始指南,只需幾個後端 API 請求即可在 sandbox 沙箱建立測試客戶、申請並啟用所需能力,並完整執行一次入金、出金或發行銀行帳戶流程。您將快速體驗 API 契約、X-API-Key 身分驗證與 webhooks 事件在真實整合情境中的實際運作方式。"
---
