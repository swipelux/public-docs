---
title: "帳戶與錢包"
description: "了解 Swipelux 的帳戶與錢包模型:比較客戶名下錢包、銀行帳戶與第三方收款人目的地三種資源的差異與用途,掌握每種入金、出金或發行銀行帳戶流程各自需要哪些帳戶,並學習如何透過 API 建立、查詢與管理這些資源,為後續的轉帳與結算流程打好穩固基礎並避免常見錯誤。"
---

每個帳戶皆歸屬於單一客戶。請從兩個面向來選擇:由誰管理,以及它是錢包還是銀行帳戶。

客戶帳戶的清單、建立、讀取、更新與封存操作位於 `/v3/customers/{customerId}/accounts` 下；單一帳戶操作會再加上 `/{accountId}`。API 不支援從 `accountId` 反查 `customerId`。請將每個 `accountId` 與對應的 `customerId` 一併儲存，讓後續請求能指定該帳戶。
...
在開始商家面板中用於卡片或 Apple Pay 的 **First pay-in** 引導之前，請先為該客戶建立 Polygon 錢包。此引導只會提供該 Polygon 錢包，沒有它便無法繼續。這項先決條件適用於商家面板引導；直接 API 整合只能使用針對目前客戶與功能所傳回的幣別、網路、方法與帳戶類型。

## 建立帳戶 {#create-the-account}
