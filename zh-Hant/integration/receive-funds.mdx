...
### 為卡片、Apple Pay 或 Google Pay 請求結帳偏好

對於卡片報價,您可以在同一請求中包含選用的 `paymentMethod`,取值為 `card`、`apple_pay` 或 `google_pay`。當該方式對客戶及其裝置可用時,託管結帳會以該方式開啟,客戶仍可選擇結帳提供的任何其他方式。省略該欄位即可保留預設偏好。
...
`paymentMethod` 是一種偏好,而不是保證。Apple Pay 與 Google Pay 是否可用取決於客戶、其裝置與結帳頁面,因此請求某種方式並不會使其變得可用。該偏好不會變更報價的能力、金額或費用,最終費用由結帳頁面確認。轉帳讀取會回傳請求的值,但不會確認客戶實際使用了哪種方式。對於非卡片報價傳送 `paymentMethod` 會導致驗證失敗,而不會被忽略;使用相同冪等鍵的重送必須傳送相同的 `paymentMethod`。
