...
`verificationSessions` 與 `tosSessions` 中的每一筆項目都會發佈一個明確的 `action` 物件。在引導客戶前,請先讀取 `action.kind`;不要從 session 的 `status` 推斷連結是否可用。

- `action.kind: "available"` 會附帶 `action.url` 與 `action.expiresAt`(目前一律為 `null`)。請將客戶引導至 `action.url`,然後再次讀取該任務。session 上扁平的 `url` 與 `expiresAt` 欄位為可用操作的相容性鏡像。
- `action.kind: "unavailable"` 表示不應向客戶顯示任何連結。已重用、已完成、已拒絕與已取消的 session 都會發佈此形態,且扁平的 `url` 與 `expiresAt` 欄位會被省略。
