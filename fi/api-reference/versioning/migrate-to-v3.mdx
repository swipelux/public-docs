...
| `POST /v2/.../documents/upload-token` plus `POST /v2/documents/direct-upload` | Lataa suoraan API-avaimellasi (alla) tai lähetä tuore direct-upload-`storageKey` JSON-muodossa osoitteeseen `POST /v3/.../documents` viiden minuutin kuluessa |
...
- **Raakatiedostojen tallennus**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (plus `/{documentId}` ja `GET .../{documentId}/download`), lataa kerran API-avaimellasi ja viittaa sitten asiakirjatunnisteisiin submission-vastauksissa. Tämä korvaa jokaisen upload-token- ja direct-upload-vastaanoton. `POST .../documents` hyväksyy myös JSON-rungon (`storageKey`, `fileName`, `type`), joka kuluttaa vielä tuoreen asiakaskohtaisen viitteen `POST /v2/documents/direct-upload`-kutsusta, joten pysäköidyt lataukset migroituvat lähettämättä tavuja uudelleen.
