...
Jokainen `verificationSessions`- ja `tosSessions`-tietueen merkintä julkaisee eksplisiittisen `action`-objektin. Lue `action.kind` ennen kuin ohjaat asiakasta eteenpäin; älä päättele linkin saatavuutta istunnon `status`-arvosta.

- `action.kind: "available"` sisältää `action.url`- ja `action.expiresAt`-arvot (tällä hetkellä aina `null`). Lähetä asiakas osoitteeseen `action.url` ja lue tehtävä sitten uudelleen. Istunnon tasoiset `url`- ja `expiresAt`-kentät ovat yhteensopivuuspeilejä saatavilla olevalle toiminnolle.
- `action.kind: "unavailable"` tarkoittaa, että linkkiä ei tule näyttää. Uudelleenkäytetyt, valmistuneet, hylätyt ja peruutetut istunnot julkaisevat tämän muodon, ja tasoiset `url`- ja `expiresAt`-kentät jätetään pois.
