...
### Pyydä kassan maksutapasuositus kortille, Apple Paylle tai Google Paylle

Korttitarjoukselle voit sisällyttää samaan pyyntöön valinnaisen `paymentMethod`-arvon `card`, `apple_pay` tai `google_pay`. Isännöity kassa avautuu kyseiseen maksutapaan, kun se on asiakkaan ja hänen laitteensa käytettävissä, ja asiakas voi silti valita minkä tahansa muun kassan tarjoaman maksutavan. Jätä arvo pois, jos haluat säilyttää oletussuosituksen.
...
`paymentMethod` on suositus, ei takuu. Apple Payn ja Google Payn saatavuus riippuu asiakkaasta, hänen laitteestaan ja kassasta, joten maksutavan pyytäminen ei tee siitä saatavilla olevaa. Suositus ei muuta tarjouksen ominaisuutta, summia tai maksuja, ja kassa vahvistaa lopulliset maksut. Siirron lukupyynnöt palauttavat pyydetyn arvon; ne eivät vahvista, mitä maksutapaa asiakas käytti. `paymentMethod`-arvon lähettäminen tarjoukselle, joka ei ole korttitarjous, epäonnistuu validoinnissa sen sijaan, että se ohitettaisiin, ja samalla idempotenssiavaimella tehdyn toiston on lähetettävä sama `paymentMethod`.
