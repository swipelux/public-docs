...
description: "Luo kertaluontoinen Swipelux-fiat-maksu, esitä tarkat pankki- tai korttiohjeet ja tilitä vastaanotetut varat asiakkaan lompakkoon."
---
...
Pankkiohjeet voivat sisältää myös valinnaiset kentät `bankName`, `bankAddress`, `accountHolderName` ja `accountHolderAddress` menetelmillä `ach`, `wire`, `rtp` ja `swift`. Näytä jokainen palautettu kenttä muiden pankkikoordinaattien vieressä. Lähettävät pankit vaativat usein vastaanottavan pankin osoitteen `swift`- ja muissa kansainvälisissä tilisiirroissa, joten näytä `bankAddress` aina, kun se palautetaan.
