...
### Esivalitse kassan maksutapa korttitarjoukselle

Korttitarjoukselle voit sisällyttää samaan pyyntöön valinnaisen `checkoutMethod`-arvon, jolla valitset, mihin rahoitusvaihtoehtoon isännöity kassa avautuu. Nykyiset arvot ovat `card`, `apple_pay`, `google_pay`, `cash_app`, `sepa` ja `blik`. Luettelo on avoin ja voi kasvaa, joten siedä siirron lukupyynnöissä arvoja, joita et tunnista. Kassa avautuu pyydettyyn maksutapaan, kun se on asiakkaan ja hänen laitteensa käytettävissä, ja asiakas voi silti valita minkä tahansa muun kassan tarjoaman maksutavan. Jätä arvo pois, jos haluat säilyttää oletusesivalinnan.
...
`checkoutMethod` on suositus, ei takuu. Saatavuus riippuu asiakkaasta, hänen laitteestaan ja kassasta, joten maksutavan pyytäminen ei tee siitä saatavilla olevaa. Tässä `sepa` ja `blik` ovat korttikassan sisäisiä rahoitusvaihtoehtoja, eivät Swipeluxin ominaisuuksia, maksureittejä tai tilejä. Esivalinta ei muuta tarjouksen ominaisuutta, summia tai maksuja, ja kassa vahvistaa lopulliset maksut. Siirron lukupyynnöt palauttavat pyydetyn arvon; ne eivät vahvista, mitä maksutapaa asiakas käytti. `checkoutMethod`-arvon lähettäminen tarjoukselle, joka ei ole korttitarjous, epäonnistuu validoinnissa sen sijaan, että se ohitettaisiin, ja samalla idempotenssiavaimella tehdyn toiston on lähetettävä sama `checkoutMethod`.
