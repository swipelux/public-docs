---
title: "账户与钱包"
description: "了解 Swipelux 中客户名下钱包、银行账户与第三方收款目的地之间的区别，并为每种入金、出金或发行银行账户流程选择所需的账户资源。本页涵盖各类账户的创建方式、关键字段说明、生命周期以及常见 API 调用示例，帮助您避免在集成中选错账户类型。请结合业务流程按需创建。"
---

每个账户都属于一个客户。请从两个维度进行选择:由谁管理,以及它是钱包还是银行账户。

客户账户的列表、创建、读取、更新和归档操作位于 `/v3/customers/{customerId}/accounts` 下；单个账户操作再追加 `/{accountId}`。API 不支持从 `accountId` 反查 `customerId`。请将每个 `accountId` 与对应的 `customerId` 一起保存，以便后续请求能够访问该账户。
...
在开始商户面板中用于银行卡或 Apple Pay 的 **First pay-in** 引导之前，请先为该客户创建 Polygon 钱包。该引导只提供这个 Polygon 钱包，没有它就无法继续。此先决条件仅适用于商户面板引导；直接 API 集成必须只使用为当前客户和能力返回的币种、网络、方法和账户类型。

## 创建账户 {#create-the-account}
