...
`verificationSessions` 和 `tosSessions` 中的每个条目都会发布一个显式的 `action` 对象。在引导客户之前请先读取 `action.kind`,不要根据会话的 `status` 推断链接是否可用。

- `action.kind: "available"` 会包含 `action.url` 和 `action.expiresAt`(目前始终为 `null`)。将客户引导到 `action.url`,然后重新读取该任务。会话上的扁平 `url` 和 `expiresAt` 字段是可用操作的兼容性镜像。
- `action.kind: "unavailable"` 表示不应向客户展示任何链接。被复用、已完成、被拒绝和已取消的会话都会发布这种形态,并且扁平的 `url` 和 `expiresAt` 字段会被省略。
