---
title: "接收资金"
description: "创建一次性的 Swipelux 法币入金转账,展示准确的银行或银行卡入金指令,并将收到的资金结算到客户钱包。"
---
...
对于银行卡或 Apple Pay 报价,您可以在同一请求中包含可选的 `redirectUrl`。当客户完成托管结账后,结账页面会打开此 URL,将客户带回您的应用。
...
### 为银行卡、Apple Pay 或 Google Pay 请求结账偏好

对于银行卡报价,您可以在同一请求中包含可选的 `paymentMethod`,取值为 `card`、`apple_pay` 或 `google_pay`。当该方式对客户及其设备可用时,托管结账会以该方式打开,客户仍可选择结账提供的任何其他方式。省略该字段即可保留默认偏好。
...
`paymentMethod` 是一种偏好,而不是保证。Apple Pay 和 Google Pay 是否可用取决于客户、其设备和结账页面,因此请求某种方式并不会使其变得可用。该偏好不会更改报价的能力、金额或费用,最终费用由结账页面确认。转账读取会回显请求的值,但不会确认客户实际使用了哪种方式。对于非银行卡报价发送 `paymentMethod` 会导致校验失败,而不会被忽略;使用相同幂等键的重放必须发送相同的 `paymentMethod`。
