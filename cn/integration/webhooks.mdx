---
title: "Webhook"
description: "了解如何校验 Swipelux 通过 Svix 签名推送的 Webhook 事件，在确认接收前先持久化唯一事件 ID，并通过重新拉取资源当前状态来安全处理重试与乱序投递。本页还介绍事件类型、端点配置以及构建可靠消费逻辑的最佳实践，是获知状态变化的推荐方式。请务必在生产环境启用。"
---
