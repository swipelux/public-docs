---
title: "收款方与目的地"
description: "了解如何通过 Swipelux API 创建第三方收款方，以及用于接收出金的银行账户或加密钱包目的地，并在后续转账中复用这些资源。本页涵盖收款方的必填字段、校验规则、支持的目的地类型以及常见错误的排查方式，收款方一经创建即可在多笔转账中重复使用。支持银行与加密两类目的地。"
---
...
`relationship` 描述收款方与你的客户之间的关系。请从 [`POST /v3/customers/{customerId}/recipients` 请求 schema](/api-reference/recipients/post-v3-customers-by-customer-id-recipients) 中选择一个值。如果发送 `other`，不要添加配套的自由文本字段；请求 schema 会拒绝未知字段。
