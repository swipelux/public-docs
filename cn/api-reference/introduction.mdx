...
API 参考是 Swipelux API 的权威契约。每个操作都会展示其请求、响应、安全要求、状态与错误结构。完整的公开 problem 目录和处理指引,请参阅[错误与重试](/cn/integration/errors)。
...
请根据该操作文档中列出的 problem 结构,来判断是需要更正输入、刷新资源状态、完成某个任务,还是进行重试。所有公开的 `code`、稳定的 problem type URI、重试行为和后续操作,请参阅[错误与重试](/cn/integration/errors)。
