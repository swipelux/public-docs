---
title: "迁移到 v3"
description: "分两步将现有的 v1 或 v2 集成迁移到 v3 Swipelux API：先理解概念，再逐个端点迁移。"
sidebarTitle: "迁移到 v3"
---

分两步将您的集成从 API v1 和 v2 迁移到 v3：
...
| `POST /v2/.../documents/upload-token` 加 `POST /v2/documents/direct-upload` | 请使用您的 API key 直接上传（见下），或在五分钟内将一个新鲜的直接上传 `storageKey` 以 JSON 形式发送到 `POST /v3/.../documents` |
...
- **原始文件存储**：`POST/GET/DELETE /v3/customers/{customerId}/documents`（以及 `/{documentId}` 和 `GET .../{documentId}/download`），使用您的 API key 一次性上传，然后在 submission 答案中引用 document id。它取代了所有的 upload-token 和 direct-upload 上传入口。`POST .../documents` 还接受一个 JSON 请求体（`storageKey`、`fileName`、`type`），用于消费来自 `POST /v2/documents/direct-upload` 的仍然新鲜的客户范围引用，因此已暂存的上传无需重新发送字节即可完成迁移。
