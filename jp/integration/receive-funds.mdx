...
### カード、Apple Pay、または Google Pay のチェックアウト優先設定をリクエストする

カードの見積もりの場合、同じリクエストにオプションの `paymentMethod`（`card`、`apple_pay`、または `google_pay`）を含めることができます。その方法が顧客とそのデバイスで利用可能な場合、ホスト型チェックアウトはその方法で開き、顧客はチェックアウトが提供する他の方法を引き続き選択できます。省略すると、デフォルトの優先設定が維持されます。
...
`paymentMethod` は優先設定であり、保証ではありません。Apple Pay と Google Pay の利用可否は顧客、そのデバイス、およびチェックアウトによって決まるため、リクエストしても利用可能になるわけではありません。優先設定は見積もりのケイパビリティ、金額、手数料を変更せず、最終的な手数料はチェックアウトで確認されます。送金の読み取りはリクエストした値を返しますが、顧客が実際に使用した方法を確認するものではありません。カードの見積もりではない見積もりに対して `paymentMethod` を送信すると、無視されるのではなくバリデーションが失敗します。また、同じ冪等キーで再送する場合は、同じ `paymentMethod` を送信する必要があります。
