...
`verificationSessions` および `tosSessions` の各エントリには、明示的な `action` オブジェクトが公開されています。顧客を誘導する前に `action.kind` を確認してください。セッションの `status` からリンクの利用可否を推測してはいけません。

- `action.kind: "available"` の場合は `action.url` と `action.expiresAt`(現時点では常に `null`)が含まれます。顧客を `action.url` に誘導し、その後タスクを再度読み取ってください。セッション上のフラットな `url` および `expiresAt` フィールドは、available アクションの互換ミラーです。
- `action.kind: "unavailable"` は、リンクを提示すべきではないことを意味します。再利用、完了、拒否、キャンセルされたセッションはこの形で公開され、フラットな `url` および `expiresAt` フィールドは省略されます。
