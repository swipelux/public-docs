---
title: "v3 への移行"
description: "既存の v1 または v2 のインテグレーションを v3 Swipelux API へ、2 つのパスで移行します。まずコンセプト、続いてエンドポイントごとに。"
...
| `POST /v2/.../documents/upload-token` に加えて `POST /v2/documents/direct-upload` | API キーで直接アップロード (以下参照)、または有効期限内の direct-upload の `storageKey` を 5 分以内に JSON として `POST /v3/.../documents` に送信 |
...
- **生ファイルストレージ**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (加えて `/{documentId}` および `GET .../{documentId}/download`)。API キーで一度アップロードし、submission の回答で document id を参照してください。これによりすべての upload-token および direct-upload の取り込みが置き換えられます。`POST .../documents` は、`POST /v2/documents/direct-upload` からのまだ有効な顧客スコープの参照を消費する JSON ボディ (`storageKey`、`fileName`、`type`) も受け付けるため、一時保管されたアップロードはバイトを再送せずに移行できます。
