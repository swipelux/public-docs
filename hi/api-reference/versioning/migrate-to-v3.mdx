...
description: "किसी मौजूदा v1 या v2 इंटीग्रेशन को दो चरणों में v3 Swipelux API पर माइग्रेट करें: पहले अवधारणा, फिर एंडपॉइंट-दर-एंडपॉइंट।"
...
| `POST /v2/.../documents/upload-token` प्लस `POST /v2/documents/direct-upload` | अपनी API key के साथ सीधे अपलोड करें (नीचे), या पाँच मिनट के भीतर `POST /v3/.../documents` पर एक ताज़ा direct-upload `storageKey` JSON के रूप में भेजें |
...
- **कच्चा फ़ाइल भंडारण**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (साथ ही `/{documentId}` और `GET .../{documentId}/download`), अपनी API key के साथ एक बार अपलोड करें, फिर submission उत्तरों में document ids का संदर्भ दें। यह हर upload-token और direct-upload intake को प्रतिस्थापित करता है। `POST .../documents` एक JSON body (`storageKey`, `fileName`, `type`) भी स्वीकार करता है जो `POST /v2/documents/direct-upload` से अभी भी ताज़ा customer-scoped reference को consume करता है, जिससे park किए गए uploads bytes दोबारा भेजे बिना माइग्रेट हो जाते हैं।
