...
`verificationSessions` और `tosSessions` में प्रत्येक entry एक explicit `action` object publish करता है। Customer को dispatch करने से पहले `action.kind` पढ़ें; session `status` से link की उपलब्धता का अनुमान न लगाएं।

- `action.kind: "available"` में `action.url` और `action.expiresAt` शामिल होते हैं (वर्तमान में हमेशा `null`)। Customer को `action.url` पर भेजें, फिर task को फिर से पढ़ें। Session पर flat `url` और `expiresAt` fields available action के compatibility mirrors हैं।
- `action.kind: "unavailable"` का अर्थ है कि कोई link प्रस्तुत नहीं किया जाना चाहिए। Reused, completed, rejected, और canceled sessions इस shape को publish करते हैं, और flat `url` तथा `expiresAt` fields को छोड़ दिया जाता है।
