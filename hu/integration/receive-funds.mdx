...
### Checkout-preferencia kérése kártyához, Apple Payhez vagy Google Payhez

Kártyás árajánlathoz ugyanabban a kérésben megadhat egy opcionális `paymentMethod` értéket: `card`, `apple_pay` vagy `google_pay`. A hosted checkout ezzel a módszerrel nyílik meg, ha az elérhető az ügyfél és az eszköze számára, és az ügyfél továbbra is választhat bármely más módszert, amelyet a checkout kínál. Hagyja el a mezőt az alapértelmezett preferencia megtartásához.
...
A `paymentMethod` preferencia, nem garancia. Az Apple Pay és a Google Pay elérhetősége az ügyféltől, az eszközétől és a checkouttól függ, így egy módszer kérése nem teszi azt elérhetővé. A preferencia nem változtatja meg az árajánlat képességét, összegeit vagy díjait, a végleges díjakat pedig a checkout erősíti meg. Az átutalás olvasása a kért értéket adja vissza; nem erősíti meg, hogy az ügyfél melyik módszert használta. Ha nem kártyás árajánlathoz küld `paymentMethod` értéket, a kérés validációs hibával meghiúsul, ahelyett hogy figyelmen kívül maradna, és az ugyanazzal az idempotenciakulccsal megismételt kérésnek ugyanazt a `paymentMethod` értéket kell küldenie.
