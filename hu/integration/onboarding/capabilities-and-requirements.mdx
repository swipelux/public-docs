...
A `verificationSessions` és `tosSessions` minden bejegyzése egy explicit `action` objektumot közöl. Olvassa be az `action.kind` értékét, mielőtt az ügyfelet továbbküldi; ne következtessen a hivatkozás elérhetőségére a munkamenet `status` értékéből.

- `action.kind: "available"` tartalmazza az `action.url` és `action.expiresAt` mezőket (jelenleg mindig `null`). Küldje el az ügyfelet az `action.url`-re, majd olvassa be újra a feladatot. A munkameneten szereplő lapos `url` és `expiresAt` mezők az elérhető művelet kompatibilitási tükörképei.
- `action.kind: "unavailable"` azt jelenti, hogy nem szabad hivatkozást megjeleníteni. Az újrafelhasznált, `completed`, elutasított és lemondott munkamenetek ezt az alakot közlik, és a lapos `url` és `expiresAt` mezők nem szerepelnek.
