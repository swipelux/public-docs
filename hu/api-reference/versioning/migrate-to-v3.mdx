...
| `POST /v2/.../documents/upload-token` plusz `POST /v2/documents/direct-upload` | Tölts fel közvetlenül az API-kulcsoddal (lent), vagy küldj egy friss direct-upload `storageKey`-t JSON-ként a `POST /v3/.../documents` végpontra öt percen belül |
...
- **Nyers fájltárolás**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (plusz `/{documentId}` és `GET .../{documentId}/download`), tölts fel egyszer az API-kulcsoddal, majd hivatkozz a dokumentum-azonosítókra a beküldési válaszokban. Ez felváltja az összes upload-token és direct-upload bevételt. A `POST .../documents` JSON-törzset is elfogad (`storageKey`, `fileName`, `type`), amely egy még friss, ügyfél-hatókörű hivatkozást használ fel a `POST /v2/documents/direct-upload` végpontról, így a parkolt feltöltések a bájtok újraküldése nélkül migrálhatók.
