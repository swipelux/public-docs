...
### Sol·licita una preferència de checkout per a targeta, Apple Pay o Google Pay

Per a una cotització de targeta, pots incloure un `paymentMethod` opcional amb el valor `card`, `apple_pay` o `google_pay` a la mateixa sol·licitud. El checkout allotjat s'obre amb aquest mètode quan està disponible per al client i el seu dispositiu, i el client encara pot triar qualsevol altre mètode que ofereixi el checkout. Omet-lo per mantenir la preferència per defecte.
...
`paymentMethod` és una preferència, no una garantia. La disponibilitat d'Apple Pay i Google Pay depèn del client, del seu dispositiu i del checkout, de manera que sol·licitar-ne un no el fa disponible. La preferència no canvia la capacitat, els imports ni les comissions de la cotització, i el checkout confirma les comissions finals. Les lectures de la transferència retornen el valor sol·licitat; no confirmen quin mètode ha utilitzat el client. Enviar `paymentMethod` per a una cotització que no és de targeta falla la validació en lloc d'ignorar-se, i una repetició amb la mateixa clau d'idempotència ha d'enviar el mateix `paymentMethod`.
