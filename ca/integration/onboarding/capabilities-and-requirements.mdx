...
Cada entrada de `verificationSessions` i `tosSessions` publica un objecte `action` explícit. Llegeix `action.kind` abans d'enviar el client; no dedueixis la disponibilitat de l'enllaç a partir del `status` de la sessió.

- `action.kind: "available"` inclou `action.url` i `action.expiresAt` (actualment sempre `null`). Envia el client a `action.url` i, tot seguit, torna a llegir la tasca. Els camps plans `url` i `expiresAt` de la sessió són miralls de compatibilitat de l'acció disponible.
- `action.kind: "unavailable"` significa que no s'ha de presentar cap enllaç. Les sessions reutilitzades, completades, rebutjades i cancel·lades publiquen aquesta forma, i s'ometen els camps plans `url` i `expiresAt`.
