...
| `POST /v2/.../documents/upload-token` més `POST /v2/documents/direct-upload` | Eliminat, puja directament amb la teva clau API (a sota) |
...
- **Emmagatzematge d'arxius en brut**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (més `/{documentId}`), puja una vegada amb la teva clau API i després referencia els ids de document a les respostes de submission. Això reemplaça tots els intakes d'upload-token i direct-upload.
