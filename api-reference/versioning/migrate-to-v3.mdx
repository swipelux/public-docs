...
| `POST /v2/.../documents/upload-token` plus `POST /v2/documents/direct-upload` | Gone, upload directly with your API key (below) |
...
- **Raw file storage**: `POST/GET/DELETE /v3/customers/{customerId}/documents` (plus `/{documentId}`), upload once with your API key, then reference document ids in submission answers. This replaces every upload-token and direct-upload intake.
