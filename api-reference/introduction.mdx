...
description: "Use the API v3 contract, retry writes safely, and handle errors consistently."
---

The API Reference is the authoritative contract for Swipelux API v3. Each operation shows its request, response, security, status, and error schemas.
...
When an API v3 operation lists `Idempotency-Key` as required, generate one key for each intended operation and store it with your local request record.
