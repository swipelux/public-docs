---
title: "API reference"
description: "Use the Swipelux v3 API contract, authenticate with X-API-Key, retry writes safely with idempotency keys, and handle status codes and error schemas."
---

The API Reference is the authoritative contract for the Swipelux API. Each operation shows its request, response, security, status, and error schemas. Use [Errors and retries](/integration/errors) for the complete public problem catalog and resolution guidance.
...
Use the operation's documented problem schemas to decide whether to correct input, refresh resource state, complete a task, or retry. See [Errors and retries](/integration/errors) for every public `code`, stable problem type URI, retry behavior, and next action.
