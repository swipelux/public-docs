...
De API Reference is het gezaghebbende contract voor de Swipelux API. Elke operation toont het request-, response-, security-, status- en foutschema. Gebruik [Fouten en nieuwe pogingen](/nl/integration/errors) voor de volledige publieke probleemcatalogus en oplossingsrichtlijnen.
...
Gebruik de gedocumenteerde probleemschema's van de operation om te beslissen of je input corrigeert, resourcestatus ververst, een taak voltooit of opnieuw probeert. Zie [Fouten en nieuwe pogingen](/nl/integration/errors) voor elke publieke `code`, stabiele problem type-URI, retry-gedrag en vervolgactie.
