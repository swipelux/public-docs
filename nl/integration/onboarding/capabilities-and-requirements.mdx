...
Wanneer de taak `verificationSessions` of `tosSessions` retourneert, bewaar dan elke sessie-`id` en huidige `url`. Stuur de klant naar de geretourneerde URL en lees de taak vervolgens opnieuw. Dit zijn task-scoped acties, geen aparte lifecycle voor klantverificatie.
