...
`relationship` beschrijft hoe de recipient zich verhoudt tot je klant. Stuur één van `employee`, `contractor`, `vendor`, `subsidiary`, `merchant`, `customer`, `landlord`, `family` of `other`. Het veld accepteert alleen deze waarden; er is geen vrije-tekstbeschrijving naast `other`.
