...
description: "Maak een eenmalige Swipelux fiat pay-in, toon de exacte bank- of kaartinstructies en verreken ontvangen gelden in de wallet van de klant."
---
...
Bankinstructies kunnen bij de methoden `ach`, `wire`, `rtp` en `swift` ook de optionele velden `bankName`, `bankAddress`, `accountHolderName` en `accountHolderAddress` bevatten. Toon elk geretourneerd veld naast de andere bankcoördinaten. Verzendende banken vereisen vaak het adres van de ontvangende bank voor `swift` en andere internationale overboekingen, dus toon `bankAddress` telkens wanneer het wordt geretourneerd.
