...
Maak een sandbox-taak met [`POST /v3/sandbox/tasks`](/api-reference/sandbox/post-v3-sandbox-tasks). Kies een `scope.type` die past bij de resource die je test en voeg één of meer `items` toe die beschrijven wat de klant moet indienen.

`scope.type` accepteert `customer`, `capability` of `transfer`. Een `transfer`-scope vereist zowel `customerId` als `transferId`. Een `capability`-scope vereist zowel `customerId` als `capabilityId`. Een `customer`-scope vereist alleen `customerId`.

`items[].type` accepteert `document`, `profile`, `questionnaire`, `explanation` en `attestation`. Gebruik `document` wanneer de klant een bestand moet uploaden, `profile` voor een specifiek klantprofielveld, `questionnaire` voor een vrij tekstantwoord, `explanation` voor een geschreven toelichting en `attestation` wanneer de klant tekst moet bevestigen.

Stel `category` in om de taak af te stemmen op een workflow. Gebruik voor een transactie Request For Information (RFI) `transaction_rfi` met een `transfer`-scope.
...
Document-`constraints` accepteert `acceptedDocumentTypes`, `acceptedFormats`, `maxAgeDays`, `captureMethods` en `nameMatch`. Zie [Sandbox-taak aanmaken](/api-reference/sandbox/post-v3-sandbox-tasks) voor de volledige veldencatalogus.
